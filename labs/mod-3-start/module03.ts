...
function calculateInterestOnlyLoanPayment( loanTerms: Loan ): string {
    const interest: number = loanTerms.interestRate / 1200;
    const payment: number = loanTerms.principal * interest;
...
function calculateConventionalLoanPayment( loanTerms: ConventionalLoan ): string {
    const interest: number = loanTerms.interestRate / 1200;
    const payment: number = loanTerms.principal * interest / (1 - (Math.pow(1 / (1 + interest), loanTerms.months)));
